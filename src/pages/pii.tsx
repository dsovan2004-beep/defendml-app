...
const piiTypeColors: Record<string, string> = {
  email: 'bg-red-500/20 text-orange-300 border-orange-400/30',
  phone: 'bg-red-500/20 text-red-300 border-red-400/30',
        ...
        <div className="max-w-7xl mx-auto px-8 py-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-gradient-to-br from-orange-500/10 to-cyan-600/10 rounded-xl p-6 border border-red-500/20">
              ...
              <div className="text-orange-300 text-sm font-medium">PII Leaks Prevented</div>
              <div className="text-orange-400/60 text-xs mt-1">Across all LLM providers</div>
            </div>
            ...
            <div className="bg-gradient-to-br from-red-500/10 to-pink-600/10 rounded-xl p-6 border border-red-500/20">
              ...
              <div className="text-red-300 text-sm font-medium">Items Redacted</div>
              <div className="text-red-400/60 text-xs mt-1">Real-time sanitization</div>
            </div>
          ...
          <div className="bg-gradient-to-r from-orange-500/10 to-cyan-500/10 border border-red-500/20 rounded-xl p-4">
          ...
          <div className="bg-gradient-to-br from-[#0A0A0A]/30 to-[#111111]/20 border border-red-500/30 rounded-xl p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
              <div>
                <p className="text-[#A0A0A0] text-sm mb-1">Accuracy</p>
                <p className="text-2xl font-bold text-green-400">99.6%</p>
                <p className="text-xs text-zinc-500 mt-1">Above 99% target ✓</p>
              </div>
              <div>
                <p className="text-[#A0A0A0] text-sm mb-1">Latency (P95)</p>
                <p className="text-2xl font-bold text-orange-400">42ms</p>
                <p className="text-xs text-zinc-500 mt-1">Below 50ms target ✓</p>
              </div>
              <div>
                <p className="text-[#A0A0A0] text-sm mb-1">Confidence Level</p>
                <p className="text-2xl font-bold text-red-400">HIGH</p>
                <p className="text-xs text-zinc-500 mt-1">Real-time scoring</p>
              </div>
              <div>
                <p className="text-[#A0A0A0] text-sm mb-1">False Positives (24h)</p>
                <p className="text-2xl font-bold text-yellow-400">8</p>
                <p className="text-xs text-zinc-500 mt-1">0.3% rate (below 1% target) ✓</p>
              </div>
            </div>

            {/* Defense Layers */}
            <div className="pt-6 border-t border-red-500/30">
              <p className="text-sm text-[#A0A0A0] mb-3">Defense Layers Active (ASL-3 Framework)</p>
                ...
                {[
                  ...
                  >
                    <div className="text-center">
                      <p className="text-xs text-[#A0A0A0]">Layer</p>
              ...
              <div className="grid grid-cols-4 gap-3 mt-2 text-xs text-[#A0A0A0]">
          ...
          <div className="bg-black/30 backdrop-blur-xl rounded-2xl p-6 border border-white/10">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className="px-4 py-2 bg-red-500/20 hover:bg-red-500/30 text-orange-300 rounded-lg text-sm font-medium transition-all border border-red-500/30"
              >
                Refresh Data
              </button>
            </div>
            <div className="flex flex-wrap gap-3">
              <button
                ...
                className={`px-4 py-3 rounded-lg border transition-all ${
                  selectedType === 'all'
                    ? 'bg-red-500 text-white border-orange-400 shadow-lg shadow-blue-500/30'
                    : 'bg-white/5 text-[#F5F5F5] border-white/10 hover:bg-white/10'
                }`}
              >
              ...
              {Object.entries(piiTypeCounts).map(([type, count]) => (
                <button
                  ...
                  className={`px-4 py-3 rounded-lg border transition-all ${
                    selectedType === type
                      ? piiTypeColors[type] || 'bg-red-500/20 text-orange-300 border-orange-400/30'
              ...
              {Object.keys(piiTypeCounts).length === 0 && (
                <div className="text-[#A0A0A0] text-sm py-3">
                  ...
                  {loading ? (
                    <tr>
                      <td colSpan={9} className="px-6 py-12 text-center">
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <span className="text-[#A0A0A0]">Loading PII events...</span>
                        </div>
                      </td>
                    </tr>
                    ...
                    <tr>
                      <td colSpan={9} className="px-6 py-12 text-center text-[#A0A0A0]">
                        <Lock className="w-12 h-12 mx-auto mb-3 text-zinc-600" />
                        <div className="font-medium">No PII events in this category</div>
                        <div className="text-sm text-zinc-500 mt-1">Your data is protected!</div>
                      </td>
                    </tr>
                  ) : (
                    filteredEvents.map((event, idx) => (
                      <tr key={(event.id as any) ?? idx} className="hover:bg-white/5 transition-all group">
                              ...
                              return (
                                <span
                                  ...
                                  className={`px-2 py-1 rounded text-xs font-medium border ${
                                    piiTypeColors[type] ||
                                    'bg-red-500/20 text-orange-200 border-red-500/30'
                                  }`}
                                >
                                  {type.replace('_', ' ')}
                                </span>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className="px-2 py-1 rounded bg-red-500/20 text-red-300 font-mono text-xs border border-red-500/30">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-gradient-to-br from-orange-500/10 to-cyan-600/10 border border-red-500/20 rounded-xl p-6">
              <div className="flex items-start gap-3">
                <AlertCircle className="w-6 h-6 text-orange-400 flex-shrink-0 mt-0.5" />
                <div>
                  <h4 className="text-sm font-semibold text-orange-300 mb-2">Data Breach Economics</h4>
