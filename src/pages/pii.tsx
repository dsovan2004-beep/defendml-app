...
import { Lock, Eye, EyeOff, AlertCircle, Shield, TrendingDown, DollarSign, FileCheck, Activity } from 'lucide-react';
...
export default function PIIPageContent(): JSX.Element {
  ...
  async function fetchPIIEvents() {
    setLoading(true);
    try {
      const apiBase = 'https://defendml-api.dsovan2004-beep.workers.dev';
      ...
      const rawLogs: ApiLog[] = Array.isArray(data?.logs) ? data.logs : Array.isArray(data?.data) ? data.data : [];

      const normalized: PiiEvent[] = rawLogs
        .map((log) => {
          const detectionsRaw: Detection[] = (log as any)?.detections && Array.isArray((log as any).detections) ? ((log as any).detections as Detection[]) : [];
          const detections = detectionsRaw.map((d) => (typeof d === 'string' ? d : d?.type || '')).filter(Boolean);

          if (!detections.some((d) => d.startsWith('pii.'))) return null;

          return {
            id: (log as any).id,
            timestamp: (log as any).timestamp || (log as any).ts || new Date().toISOString(),
            detections,
  ...
  const piiTypeCounts: Record<string, number> = piiEvents.reduce((acc: Record<string, number>, event: PiiEvent) => {
  ...
  const totalBreachCostPrevented = piiEvents.reduce((sum, e) => sum + (e.cost_of_breach_prevented || 0), 0);

  const filteredEvents = selectedType === 'all' ? piiEvents : piiEvents.filter((e) => e.detections.some((d) => d === `pii.${selectedType}`));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-slate-950">
        ...
        <div className="max-w-7xl mx-auto px-8 py-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-slate-300">Prevent data leaks before they happen with AI-powered detection</p>
              <p className="text-slate-500 text-sm mt-1">
                <span className="text-green-400 font-semibold">Lakera doesn't show this:</span> Real-time breach cost prevention
              </p>
            </div>
            <button onClick={() => setShowSensitive((s) => !s)} className="flex items-center gap-2 px-4 py-2 bg-blue-500/20 hover:bg-blue-500/30 border border-blue-500/30 rounded-lg transition-all">
              {showSensitive ? <EyeOff className="w-4 h-4 text-blue-300" /> : <Eye className="w-4 h-4 text-blue-300" />}
              <span className="text-sm font-medium text-blue-300">{showSensitive ? 'Hide' : 'Show'} Sensitive Details</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-8 py-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-gradient-to-br from-orange-500/10 to-red-600/10 rounded-xl p-6 border border-orange-500/20">
            ...
            <div className="text-4xl font-bold text-white mb-1">${(totalBreachCostPrevented / 1000).toFixed(1)}K</div>
        ...
        <div className="bg-gradient-to-r from-blue-500/10 to-cyan-500/10 border border-blue-500/20 rounded-xl p-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="text-sm font-semibold text-blue-300 mb-1">💎 DefendML Exclusive: Breach Cost Prevention Tracking</h4>
              <p className="text-sm text-blue-200/80">
                We calculate the average cost of each PII type breach (based on IBM's 2024 Cost of Data Breach Report). See exactly how much money you're saving. <span className="font-bold">No competitor offers this transparency.</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-purple-900/30 to-purple-800/20 border border-purple-500/30 rounded-xl p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="px-3 py-1 rounded-full text-sm font-medium bg-green-500/20 text-green-300 border border-green-500/30">ACTIVE</span>
          </div>
          ...
          <div className="pt-6 border-t border-purple-500/30">
            ...
            <div className="flex gap-3">
              {[
                ...
                <div key={layer.num} className="flex items-center justify-center w-16 h-16 rounded-lg border-2 border-green-500 bg-green-500/10">
        ...
        <div className="bg-black/30 backdrop-blur-xl rounded-2xl p-6 border border-white/10">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={fetchPIIEvents} className="px-4 py-2 bg-blue-500/20 hover:bg-blue-500/30 text-blue-300 rounded-lg text-sm font-medium transition-all border border-blue-500/30">
              Refresh Data
            </button>
          </div>
          <div className="flex flex-wrap gap-3">
            <button onClick={() => setSelectedType('all')} className={`px-4 py-3 rounded-lg border transition-all ${selectedType === 'all' ? 'bg-blue-500 text-white border-blue-400 shadow-lg shadow-blue-500/30' : 'bg-white/5 text-slate-300 border-white/10 hover:bg-white/10'}`}>
            ...
            {Object.entries(piiTypeCounts).map(([type, count]) => (
              <button key={type} onClick={() => setSelectedType(type)} className={`px-4 py-3 rounded-lg border transition-all ${selectedType === type ? piiTypeColors[type] || 'bg-blue-500/20 text-blue-300 border-blue-400/30' : 'bg-white/5 text-slate-400 border-white/10 hover:bg-white/10'}`}>
                <div className="text-xs uppercase tracking-wide mb-1">{type.replace('_', ' ')}</div>
                <div className="text-2xl font-bold">{count}</div>
              </button>
            ))}
            {Object.keys(piiTypeCounts).length === 0 && <div className="text-slate-400 text-sm py-3">No PII types detected yet - your data is secure! 🎉</div>}
          </div>
        </div>
        ...
        <div className="bg-black/30 backdrop-blur-xl rounded-2xl border border-white/10 overflow-hidden">
          <div className="px-6 py-4 border-b border-white/10 bg-white/5">
            ...
            <p className="text-slate-400 text-sm">Real-time data leak prevention across all your LLM providers</p>
          </div>
            ...
            <table className="w-full">
              <thead className="bg-white/5 border-b border-white/10">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">PII Types</th>
                  <th className="px-6 py-4 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">Confidence</th>
                  <th className="px-6 py-4 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">Layer</th>
                  <th className="px-6 py-4 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">Items Redacted</th>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">Compliance</th>
                  <th className="px-6 py-4 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">Cost Saved</th>
                </tr>
              </thead>
                  ...
                  filteredEvents.map((event, idx) => (
                    <tr key={(event.id as any) ?? idx} className="hover:bg-white/5 transition-all">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-slate-300">{new Date(event.timestamp).toLocaleTimeString()}</div>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-wrap gap-1">
                          {(event.detections || []).map((d, i) => {
                            const type = d.replace('pii.', '');
                            return (
                              <span key={i} className={`px-2 py-1 rounded text-xs font-medium border ${piiTypeColors[type] || 'bg-blue-500/20 text-blue-200 border-blue-500/30'}`}>
                                {type.replace('_', ' ')}
                              </span>
                            );
                          })}
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <div className="w-16 h-2 bg-slate-700 rounded-full overflow-hidden">
                            <div className="h-full bg-green-500" style={{ width: `${event.confidence_score || 95}%` }}></div>
                          </div>
                          <span className="text-sm font-bold text-green-400">{(event.confidence_score || 95).toFixed(1)}%</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-2xl font-bold text-blue-400">{event.redacted_count || 0}</span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                          <span className="text-sm text-slate-300 font-medium">{event.llm_provider || 'Unknown'}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-3 py-1.5 rounded-lg text-xs font-bold border ${event.action === 'blocked' ? 'bg-red-500/20 text-red-300 border-red-500/30' : 'bg-yellow-500/20 text-yellow-300 border-yellow-500/30'}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-sm font-bold text-green-400">${(event.cost_of_breach_prevented || 0).toLocaleString()}</span>
                        </div>
                      </td>
                    </tr>
                  ))
          ...
          <div className="bg-gradient-to-br from-green-500/10 to-emerald-600/10 border border-green-500/20 rounded-xl p-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-green-200/80 mb-3">All PII detection events are automatically logged for SOC 2, GDPR, and HIPAA compliance audits.</p>
          ...
          <div className="bg-gradient-to-br from-blue-500/10 to-cyan-600/10 border border-blue-500/20 rounded-xl p-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-blue-200/80 mb-2">Average cost per PII record breach (IBM 2024 Report):</p>
      ...
      <footer className="bg-slate-950 border-t border-white/10 py-6 mt-12">
        <div className="max-w-7xl mx-auto px-8 flex items-center justify-between text-sm text-slate-400">
          <div>© 2025 DefendML</div>
