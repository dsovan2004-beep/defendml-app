      ...
      <div className="flex-1 bg-gradient-to-br from-slate-950 via-red-950 to-slate-950 flex items-center justify-center">
        <div className="text-center p-8 rounded-2xl bg-black/30 border border-white/10 backdrop-blur-xl max-w-md">
          ...
          <p className="mt-4 text-sm text-slate-400">
            ...
            <br />
            Required role:{" "}
            <span className="text-purple-400">super_admin or admin</span>
          </p>
          <button
            onClick={() => router.push("/overview")}
...
function PIIPageContent(): JSX.Element {
  ...
  async function fetchPIIEvents() {
    setLoading(true);
    try {
      const rel = `/api/logs/recent?limit=500`;
      const extBase =
        process.env.NEXT_PUBLIC_API_BASE ||
        "https://defendml-api.dsovan2004-beep.workers.dev";
      const ext = `${extBase.replace(/\/$/, "")}/api/logs/recent?limit=500`;

      let res = await fetch(rel, { credentials: "omit" }).catch(() => null);
      if (!res || !res.ok) {
        res = await fetch(ext, { credentials: "omit" }).catch(() => null);
      }

      if (!res || !res.ok) throw new Error("Failed to fetch PII events");
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 bg-gradient-to-br from-slate-950 via-blue-950 to-slate-950 p-8 text-white">
        ...
        <p className="text-slate-400 mb-8">
          Detect, redact, and track personally identifiable information (PII)
        ...
        {loading ? (
          ...
          piiEvents.slice(0, 10).map((e, idx) => (
            <div
              key={idx}
              className="p-4 mb-4 rounded-xl bg-black/30 border border-white/10"
            >
              <div className="flex justify-between">
                ...
                <div className="text-right text-xs text-slate-400 uppercase">
                  {e.action}
                </div>
              </div>
