...
export default function ThreatsPage() {
  ...
  const fetchThreats = async () => {
    ...
    try {
      ...
      const threatLogs = data.logs?.filter(log => {
        const detections = log.detections || [];
        return detections.some(d => 
          ['prompt_injection', 'jailbreak', 'indirect', 'encoding', 'policy_violation'].includes(d)
        );
      }) || [];
  ...
  const getActionBadge = (action) => {
    switch(action?.toLowerCase()) {
      ...
      case 'sanitized': return 'bg-yellow-500/20 text-yellow-300 border border-yellow-500/30';
      case 'allowed': return 'bg-green-500/20 text-green-300 border border-green-500/30';
  ...
  const avgResponseTime = threats.length > 0 
    ? Math.round(threats.reduce((sum, t) => sum + (t.response_time_ms || 0), 0) / threats.length)
    : 0;
                    ...
                    <tr key={threat.id || idx} className="hover:bg-white/5 transition-all group">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-wrap gap-1">
                          {(threat.detections || []).map((detection, i) => (
                            <span key={i} className="px-2 py-1 bg-purple-500/20 text-purple-200 rounded text-xs font-medium border border-purple-500/30">
                              {detection.replace('_', ' ')}
                            </span>
                          ))}
                        </div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-sm font-bold text-green-400">{threat.response_time_ms}ms</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
